...
namespace DXGame.Services.Game.Domain.Models
{
    public class Game
    {
        ...
        public void AddPlayer(Guid id) 
        {
            if (_players.Any(p => p == id))
                throw new DXGameException("game_already_contains_specified_player");
        ...
        {
            if (!_players.Any(p => p == id))
                throw new DXGameException("game_does_not_contain_specified_player");
        ...
        public void AddRound(Guid id) 
        {
            if (_rounds.Any(r => r == id))
                throw new DXGameException("game_already_contains_specified_round");
