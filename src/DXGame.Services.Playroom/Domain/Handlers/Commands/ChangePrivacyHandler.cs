...
namespace DXGame.Services.Playroom.Domain.Handlers.Commands
{
    public class ChangePrivacyHandler : ICommandHandler<ChangePrivacy>
    {
        ...
        public async Task HandleAsync(ChangePrivacy command) => await _handler
            ...
            {
                await _eventService.PublishEventsAsync(new PrivacyChangeFailed(command.Playroom, ex.ErrorCode));
            })
            .OnError(async ex => 
            {
                await _eventService.PublishEventsAsync(new PrivacyChangeFailed(command.Playroom, ex.GetType().Name));
            })
