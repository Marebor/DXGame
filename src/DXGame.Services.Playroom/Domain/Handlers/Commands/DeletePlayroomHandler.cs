...
namespace DXGame.Services.Playroom.Domain.Handlers.Commands
{
    public class DeletePlayroomHandler : ICommandHandler<DeletePlayroom>
    {
        ...
        public async Task HandleAsync(DeletePlayroom command) => await _handler
            ...
            {
                await _eventService.PublishEventsAsync(new PlayroomDeletionFailed(command.Playroom, ex.ErrorCode));
            })
            .OnError(async ex => 
            {
                await _eventService.PublishEventsAsync(new PlayroomDeletionFailed(command.Playroom, ex.GetType().Name));
            })
