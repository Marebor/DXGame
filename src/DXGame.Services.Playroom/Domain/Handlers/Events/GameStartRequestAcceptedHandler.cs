...
namespace DXGame.Services.Playroom.Domain.Handlers.Events
{
    public class GameStartRequestAcceptedHandler : IEventHandler<GameStartRequestAccepted>
    {
        ...
        public async Task HandleAsync(GameStartRequestAccepted @event) => await _handler
            .LoadAggregate(async () =>
            {
            ...
            {
                await _eventService.PublishEventsAsync(new GameStartFailed(@event.Playroom, @event.Game, ex.ErrorCode, null));
            })
            .OnError(async ex => 
            {
                await _eventService.PublishEventsAsync(new GameStartFailed(@event.Playroom, @event.Game, ex.GetType().Name, null));
            })
